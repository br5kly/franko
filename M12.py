...
def chk():
  ...
  try: 
    ...
    httpCaht = requests.get("https://pastebin.com/XpsehWGu").text
...
def poster():
    ...
    l6 = ('     %s Multi Brute Force Facebook %s%s %sBy %sZ3Y4D     '%(P,J,Version,P,J))
    print('%s\n%s\n%s\n%s\n%s\n%s'%(l1,l2,l3,l4,l5,l6))
def poster2():
    l1 = ('     %s  _________       __                          '%(P))
    ...
    l6 = ('     %s Multi Brute Force Facebook %s%s %sBy %sZ3Y4D     '%(P,H,Version,P,H))
    print('%s\n%s\n%s\n%s\n%s\n%s'%(l1,l2,l3,l4,l5,l6))
def poster3():
    l1 = ('     %s  _________       __                          '%(P))
    ...
    l6 = ('     %s Multi Brute Force Facebook %s%s %sBy %sZ3Y4D     '%(P,B,Version,P,B))
    print('%s\n%s\n%s\n%s\n%s\n%s'%(l1,l2,l3,l4,l5,l6))
...
def tampilan_menu():
    ...
    try:open('tool/useragent.json','r').read()
        ...
        poster2()
        print('')
        tamp_new = (f'   {P2}this tool by Z3Y4D AL3BANY MY TELE=> https://t.me/sxtz0 !\n\n                {H2}- ZEYAD ALABANY -')
...
def pilih_menu():
    ...
    if dc in ['1','01','a']    : gabung_sandi.append(Author);publik();system_login();urut_crack();pilihan_sakdurunge_crack();addpass();crack()
        ...
        poster3()
        print('')
        tamp_logout1 = (f'   {P2}الاداه الاصليه احذرو تقليد \n\n                {B2}- jihad -')
        tamp_logout2 = f'''{P2}THIS TOOL BY BR3K :
    {B2}• {P2}Token/Cookies
    {B2}• {P2}File Dump
    {B2}• {P2}File Tools'''
